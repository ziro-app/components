...
export namespace UseFullOCR {
    ...
    export function transformResult<R extends ClassResult>(result: R): ClassToData<R> {
        ...
        const validations = Object.entries(v).reduce((acc,[vName,vResult]) => {
            const result = vResult.passed === true ?
                { passed: true } : 
                { passed: false, reason: vResult.reason.getData() }
     ...
     */
    function discriminator<R extends KnownDocument>(obj: DataResult,checker: (obj: any) => obj is R): obj is DataResult<R> {
