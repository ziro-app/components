import { checkKeys, checkTypes } from "../utils";
 ...
 * BackgroundCheck
 */
function isBGCheck<T>(obj: any): obj is _BGCheck.Generic<T> {
  return checkKeys(_BGCheck.BackgroundCheckKeys, obj);
}

export const BackgroundCheck: _BGCheck.TypeCheck = function (
  obj
): obj is _BGCheck.BackgroundCheck.BackgroundCheck {
  const keys = ["found", "passedOn"];
  if (!checkKeys(keys, obj)) return false;
  const { found, passedOn } = obj;
  if (!BackgroundCheck.Found(found)) return false;
  if (!BackgroundCheck.PassedOn(passedOn)) return false;
  return true;
};

BackgroundCheck.Found = function (obj): obj is _BGCheck.BackgroundCheck.Found {
  return isBGCheck<string>(obj) && checkTypes("string", obj);
};

BackgroundCheck.PassedOn = function (
  obj
): obj is _BGCheck.BackgroundCheck.PassedOn {
  return isBGCheck<boolean>(obj) && checkTypes("boolean", obj);
};
