...
const prepareDataWithInsurance = (
    ...
    try {
        ...
        const [antiFraud, markup] = isNewPlan
            ? mountSplitRules({ sellerZoopPlan, cardBrand, installments, insurance, split_rules })
...
export default (
    ...
    try {
        ...
        return insurance
            ? prepareDataWithInsurance(transaction, firebaseDocument, storeowner, timestamp, cardBrand, transactionComplete)
            : prepareDataWithoutInsurance(transaction, firebaseDocument, storeowner, timestamp, receivables, cardBrand, transactionComplete);
    } catch (e) {
        //console.log("error on prepareDataToDbAndSheet", e);
    }
};
