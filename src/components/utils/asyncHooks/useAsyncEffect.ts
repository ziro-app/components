...
const showingMessage = <H extends UAE|URAE>(hook: H) => function <R,E>(
  ...
  const newPromise = useMemo(() => {
    ...
    return ((mountState) => new Promise((resolve,reject) => {
      setMessage(message.withPromise(promise(mountState).then(resolve,reject)))
